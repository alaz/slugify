...
class Slugify(algo: String) {
  ...
  protected def preprocess(s: String) = s.trim.replaceAll("ß", "ss")
...
/** This makes pretty slugs out of any string in any charset.
  *
  * There is an alternative implementation at https://github.com/slugify/slugify , but
  * - it has GNU license
  ...
  */
object Slugify {
  val ASCII = """Any-Latin; NFD; [:M:] Remove; NFC; [^\p{ASCII}\p{Space}] Remove"""
